...
export default async function MatchCard({ match }: any) {
    return (
        <div className='w-full flex justify-between gap-2 2xl:gap-4 border border-primary rounded-md 2xl:rounded-lg p-3.5 2xl:p-4'>
            ...
            <div className='w-full flex flex-col justify-center items-center gap-2.5'>
                <Image src={match?.teamOne?.logo} alt="Bercelona team" width={45} height={45} className="w-10 2xl:w-12 h-10 2xl:h-12 rounded-full" />
                <p className="text-sm">{match?.teamOne?.name}</p>
            ...
            <div className='w-full flex flex-col justify-center items-center gap-1.5'>
                <p className="text-white/50 text-[9px] 2xl:text-[10px]">{match?.leagueName}</p>
                ...
                <div className='flex items-center gap-0.5'>
                    ...
                    <span className="text-white/50 text-sm">{match?.timeLeft}</span>
                </div>
                <div className='w-full mt-auto flex items-center justify-center bg-white text-black font-medium rounded text-sm py-1'>
                    <p>{match?.ratioTwo}</p>
                </div>
            ...
            {/* this is first match div  */}
            <div className='w-full flex flex-col justify-center items-center gap-2.5'>
                <Image src={match?.teamTwo?.logo} alt="Bercelona team" width={45} height={45} className="w-10 2xl:w-12 h-10 2xl:h-12 rounded-full" />
                <p className="text-sm">{match?.teamTwo?.name}</p>
                <div className='w-full mt-auto flex items-center justify-center bg-white text-black font-medium rounded text-sm py-1'>
                    <p>{match?.ratioThree}</p>
                </div>
            </div>
        </div>
    )
